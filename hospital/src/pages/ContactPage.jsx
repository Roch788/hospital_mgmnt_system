...
import { HelpCircle, Mail, MapPin, MessageSquare, Phone, PhoneCall, User, UserCheck } from 'lucide-react';
import FloatingNavbar from '../components/FloatingNavbar';
import Footer from '../components/Footer';

const contactCards = [
  {
    icon: <PhoneCall className="text-xl text-blue-600" />,
    title: 'Phone Support',
    value: '+91 XXX XXX XXXX',
  },
  {
    icon: <Mail className="text-xl text-blue-600" />,
    title: 'Email Support',
        ...
        <section className="grid grid-cols-1 gap-8 lg:grid-cols-5">
          ...
          >
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              <InputField icon={<User />} placeholder="Full Name" value={formData.fullName} onChange={(value) => setFormData((prev) => ({ ...prev, fullName: value }))} />
              <InputField icon={<Mail />} type="email" placeholder="Email Address" value={formData.email} onChange={(value) => setFormData((prev) => ({ ...prev, email: value }))} />
              <InputField icon={<PhoneCall />} type="tel" placeholder="Mobile Number" value={formData.mobile} onChange={(value) => setFormData((prev) => ({ ...prev, mobile: value }))} />
              <SelectField
                icon={<UserCheck />}
            ...
            <div className="mt-4 space-y-4">
              <InputField icon={<MessageSquare />} placeholder="Subject" value={formData.subject} onChange={(value) => setFormData((prev) => ({ ...prev, subject: value }))} />
              <div className="relative">
                <MessageSquare className="pointer-events-none absolute left-4 top-4 text-blue-500" />
                <textarea
                  required
              ...
              <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50/70 p-4">
                <div className="relative h-24">
                  ...
                  <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 50" preserveAspectRatio="none">
                    <path d="M15 24 L46 13 L80 28" fill="transparent" stroke="rgba(59,130,246,0.65)" strokeWidth="1.2" />
                  </svg>
                </div>
              </div>
            </section>
          ...
          <div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm">
            ...
            <div className="relative mt-5 overflow-hidden rounded-2xl border border-blue-100">
              ...
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                ...
                />
                <MapPin className="relative text-3xl text-red-600" />
              </div>
            </div>
          </div>
              ...
              {faqItems.map((item, index) => {
                ...
                return (
                  <div key={item.q} className="rounded-2xl border border-blue-100 bg-blue-50/40 overflow-hidden">
                    ...
                    >
                      <span className="font-medium text-gray-800 flex items-center gap-2"><HelpCircle className="text-blue-600" />{item.q}</span>
